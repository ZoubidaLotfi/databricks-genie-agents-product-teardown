...
Day 2 defines the Wanderbricks data that the dashboard and Databricks Genie Agent will use.

The goal is not to create a production-grade data model. The goal is to establish a clear and testable foundation for the teardown:
...
Create the reusable SQL queries in:

```text
sql/01-core-business-metrics.sql
```

Then use those queries to build the first dashboard baseline before configuring the Genie Agent.
